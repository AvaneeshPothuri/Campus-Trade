...
export default function Auctions({ user }) {
  ...
  const fetchAuctions = async () => {
    ...
    setAuctions(data || [])
    auctionsRef.current = data || []

    data.forEach(a => fetchBidsForAuction(a.auction_id))
  }
  ...
  const createAuction = async () => {
    ...
    await supabase.from('auctions').insert([{
      ...
      start_price: parseFloat(newAuction.startPrice),
      current_price: parseFloat(newAuction.startPrice),
  ...
  const handleBid = async () => {
    ...
    const current = parseFloat(currentAuction.current_price)

    if (isNaN(bid)) return alert('Enter a valid number')
    if (bid <= current) {
            ...
            {auctions.map(auction => (
              <Card key={auction.auction_id}>
                ...
                <p className="text-sm text-gray-500 mb-2">
                  Ends at: {formatLocalTime(auction.end_time)}
                </p>

                {auction.seller_username === user ? (
                  <>
                    <h5 className="mt-4 font-semibold text-gray-700">Your Auction's Bid History:</h5>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
            ...
            <div className="mt-4 flex justify-end space-x-3">
              ...
              <Button color="green" onClick={handleBid}>Submit Bid</Button>
            </div>
          </div>
        </div>
      )}
